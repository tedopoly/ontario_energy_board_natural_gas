    ...
    async def _async_update_data(self) -> None:
        ...
        for company in tree.findall("GasBillData"):
            ...
            if current_company == self.energy_company:
                ...
                self.delivery_charge_tier_1 = float(company.find(XML_KEY_DELIVERY_CHARGE_TIER_1).text)
                self.delivery_charge_tier_2 = float(company.find(XML_KEY_DELIVERY_CHARGE_TIER_2).text)
                self.delivery_charge_tier_3 = float(company.find(XML_KEY_DELIVERY_CHARGE_TIER_3).text)
                self.delivery_charge_tier_4 = float(company.find(XML_KEY_DELIVERY_CHARGE_TIER_4).text)
                self.delivery_tier_1_start = float(company.find(XML_KEY_DELIVERY_TIER_1_START).text)
                self.delivery_tier_1_end = float(company.find(XML_KEY_DELIVERY_TIER_1_END).text)
                self.delivery_tier_2_start = float(company.find(XML_KEY_DELIVERY_TIER_2_START).text)
                self.delivery_tier_2_end = float(company.find(XML_KEY_DELIVERY_TIER_2_END).text)
                self.delivery_tier_3_start = float(company.find(XML_KEY_DELIVERY_TIER_3_START).text)
                self.delivery_tier_3_end = float(company.find(XML_KEY_DELIVERY_TIER_3_END).text)
                self.delivery_tier_4_start = float(company.find(XML_KEY_DELIVERY_TIER_4_START).text)
                self.delivery_tier_4_end = float(company.find(XML_KEY_DELIVERY_TIER_4_END).text)
                return
